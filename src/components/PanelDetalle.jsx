...
// PATRÔ�: campos vacíos invisibles
function Campo({ label, valor }) {
  if (!valor && valor !== 0) return null;
  return (
    <div style={{ marginBottom: 8 }}>
      <Lbl>{label}</Lbl>
      <div style={{ fontSize: 13, color: "#374151" }}>{valor}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid #f3f4f6", flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 600, color: "#111" }}>Observaciones y preguntas</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
        {obs.map(function(o, i) {
          return (
            <div key={i} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: o.esGestor ? colors.accion : "#111", marginBottom: 4 }}>{o.autor}</div>
              <div style={{ fontSize: 13, color: "#374151", lineHeight: 1.5 }}>{o.texto}</div>
              <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>{o.fecha}</div>
            </div>
          );
        })}
      </div>
  ...
  function EconRow(props) {
    ...
    return (
      <tr style={{ borderTop: "1px solid #f3f4f6" }}>
        ...
        <td style={{ textAlign: "right", fontWeight: props.bold ? 700 : 400, color: props.bold ? colors.roiHighlight : "#111", fontSize: 12 }}>{an}</td>
      </tr>
    );
  }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", display: "flex", flexDirection: "column", height: "100%" }}>
                ...
                {SECCIONES.map(function(s) {
                  return (
                    <div key={s.id} onClick={function() { goTo(s.id); }}
                      style={{ padding: "9px 16px", fontSize: 13, color: seccion === s.id ? colors.accion : "#374151", fontWeight: seccion === s.id ? 600 : 400, cursor: "pointer", background: seccion === s.id ? colors.accionLight : "transparent" }}>
                      {s.label}
                    </div>
                  );
                })}
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "0 20px 80px" }}>
        ...
        <div ref={function(el) { refs.current["equipo"] = el; }}>
          ...
          {/* Equipo profesional — campos vacíos no se muestran */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
             ...
             .map(function(x) {
               return <div key={x[0]}><Lbl>{x[0]}</Lbl><div style={{ fontSize: 13, color: "#374151" }}>{x[1]}</div></div>;
             })}
          </div>
        </div>
              ...
              {[["Pesimista", d.escenarios.pesimista, "#fef3c7"], ["Realista", d.escenarios.realista, "#f0fdf4"], ["Optimista", d.escenarios.optimista, "#eff6ff"]].map(function(e) {
                return (
                  <div key={e[0]} style={{ background: e[2], borderRadius: 8, padding: "10px 12px", textAlign: "center" }}>
                    <Lbl>{e[0]}</Lbl>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#111" }}>{e[1]} EUR</div>
                  </div>
                );
              })}
            </div>
        ...
        <div ref={function(el) { refs.current["precio"] = el; }}>
            ...
            {pv.paso0 && pv.paso0.cargas && pv.paso0.cargas.filter(function(c) { return !!c.acreedor; }).length > 0 && (
              <div style={{ marginTop: 8 }}>
                ...
                {pv.paso0.cargas.filter(function(c) { return !!c.acreedor; }).map(function(c, i) {
                  return <div key={i} style={{ fontSize: 12, color: "#374151" }}>{c.acreedor}: {c.importe}</div>;
                })}
              </div>
            )}
          </div>
          ...
          {pv.paso4 && (
            <div>
              ...
              <Campo label="Fecha de publicación"   valor={pv.paso4.fechaPublicacion} />
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, whiteSpace: "nowrap" }}>
                    ...
                    <tbody>
                      ...
                      <tr style={{ borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td></td>
                        {pv.paso4.dias.map(function(dia, i) {
                          return <td key={i} style={{ padding: "4px 8px", textAlign: "right", color: "#9ca3af", fontSize: 10 }}>{dia.fecha}</td>;
                        })}
                      </tr>
                    </tbody>
                  </table>
            ...
            {d.documentos.filter(function(doc) { return !!doc.nombre; }).map(function(doc, i) {
              return (
                <button key={i}
                  ...
                  <span style={{ fontSize: 13, color: "#374151", fontWeight: 500 }}>{doc.nombre}</span>
                </button>
              );
            })}
          </div>
          ...
          {d.observaciones.slice(0, 2).map(function(o, i) {
            return (
              <div key={i} style={{ padding: "10px 0", borderBottom: "1px solid #f3f4f6" }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: o.esGestor ? colors.accion : "#111", marginBottom: 3 }}>{o.autor}</div>
