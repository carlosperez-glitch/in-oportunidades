...
function Lbl({ children }) {
  return <div style={{ fontSize: 11, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 3 }}>{children}</div>;
}
function Divider() {
  return <div style={{ height: 1, background: "#f3f4f6", margin: "18px 0" }} />;
}
function SecTitle({ children }) {
  return <div style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.06em" }}>{children}</div>;
}
function Txt({ text }) {
  return <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.6, whiteSpace: "pre-line" }}>{text}</div>;
}
...
function Campo({ label, valor }) {
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <Lbl>{label}</Lbl>
      <div style={{ fontSize: 14, color: "#374151" }}>{valor}</div>
    </div>
  );
}
// ─── Lightbox ─────────────────────────────────────────────────────────────────
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div onClick={function(e) { e.stopPropagation(); }} style={{ background: "#fff", borderRadius: 10, padding: "28px 32px", minWidth: 280, textAlign: "center", position: "relative" }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: "#111", marginBottom: 12 }}>{items[index]}</div>
        <div style={{ width: 240, height: 160, background: "#f3f4f6", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", color: "#9ca3af", fontSize: 13, margin: "0 auto 16px" }}>Documento</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <button onClick={onPrev} disabled={index === 0} style={{ border: "1.5px solid #e5e7eb", borderRadius: 6, padding: "5px 12px", cursor: index === 0 ? "default" : "pointer", background: "#fff", color: index === 0 ? "#d1d5db" : "#374151", fontFamily: "inherit", fontSize: 13 }}>Anterior</button>
          <span style={{ fontSize: 12, color: "#9ca3af" }}>{index + 1} / {items.length}</span>
          <button onClick={onNext} disabled={index === items.length - 1} style={{ border: "1.5px solid #e5e7eb", borderRadius: 6, padding: "5px 12px", cursor: index === items.length - 1 ? "default" : "pointer", background: "#fff", color: index === items.length - 1 ? "#d1d5db" : "#374151", fontFamily: "inherit", fontSize: 13 }}>Siguiente</button>
        </div>
        <button onClick={onClose} style={{ position: "absolute", top: 8, right: 12, background: "none", border: "none", fontSize: 21, cursor: "pointer", color: "#9ca3af" }}>x</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid #f3f4f6", flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 15, fontWeight: 600, color: "#111" }}>Observaciones y preguntas</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
        {obs.map(function(o, i) {
          return (
            <div key={i} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: o.esGestor ? colors.accion : "#111", marginBottom: 4 }}>{o.autor}</div>
              <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.5 }}>{o.texto}</div>
              <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>{o.fecha}</div>
            </div>
          );
        })}
      </div>
      <div style={{ padding: "12px 20px", borderTop: "1px solid #f3f4f6", display: "flex", gap: 8, flexShrink: 0, background: "#fff" }}>
        <input value={input} onChange={function(e) { setInput(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter") enviar(); }}
          ...
          style={{ flex: 1, padding: "10px 14px", border: "1.5px solid #e5e7eb", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none" }} />
        <button onClick={enviar} style={{ padding: "10px 18px", background: colors.accion, color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, fontFamily: "inherit", cursor: "pointer" }}>Enviar</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid #f3f4f6", flexShrink: 0 }}>
        ...
        <span style={{ fontSize: 15, fontWeight: 600, color: "#111" }}>Reseñas de la operación</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #f3f4f6" }}>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 19, fontWeight: 700, color: "#111" }}>{media}</span>
              <Stars n={parseFloat(media)} />
            </div>
          </div>
          <div style={{ borderLeft: "1px solid #f3f4f6", paddingLeft: 16 }}>
            <Lbl>Total reseñas</Lbl>
            <div style={{ fontSize: 19, fontWeight: 700, color: "#111" }}>{resenas.length}</div>
          </div>
        </div>
          ...
          return (
            <div key={i} style={{ marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #f3f4f6" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: "#111" }}>{r.autor}</span>
                <span style={{ fontSize: 12, color: "#9ca3af" }}>{r.fecha}</span>
              </div>
              <div style={{ marginBottom: 3 }}><Stars n={r.rating} /></div>
              <div style={{ fontSize: 14, color: "#374151" }}>{r.texto}</div>
            </div>
          );
        })}
      </div>
      <div style={{ padding: "12px 20px", borderTop: "1px solid #f3f4f6", flexShrink: 0, background: "#fff" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input value={inputText} onChange={function(e) { setInputText(e.target.value); }} onKeyDown={function(e) { if (e.key === "Enter") valorar(); }}
            ...
            style={{ flex: 1, padding: "10px 14px", border: "1.5px solid #e5e7eb", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none" }} />
          <button onClick={valorar} style={{ padding: "10px 18px", background: colors.accion, color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, fontFamily: "inherit", cursor: "pointer" }}>Valorar</button>
        </div>
      </div>
    </div>
  );
  ...
  function EconRow(props) {
    ...
    return (
      <tr style={{ borderTop: "1px solid #f3f4f6" }}>
        <td style={{ padding: "5px 0", fontWeight: 500, color: "#374151", fontSize: 13 }}>{props.label}</td>
        <td style={{ textAlign: "right", color: "#6b7280", fontSize: 13 }}>{props.ap != null ? props.ap : ""}</td>
        <td style={{ textAlign: "right", color: "#6b7280", fontSize: 13 }}>{props.apK}</td>
        <td style={{ textAlign: "right", color: "#6b7280", fontSize: 13 }}>{props.rep != null ? props.rep : ""}</td>
        <td style={{ textAlign: "right", color: "#6b7280", fontSize: 13 }}>{props.repK}</td>
        <td style={{ textAlign: "right", fontWeight: props.bold ? 700 : 400, color: "#111", fontSize: 13 }}>{abs}</td>
        <td style={{ textAlign: "right", fontWeight: props.bold ? 700 : 400, color: props.bold ? colors.roiHighlight : "#111", fontSize: 13 }}>{an}</td>
      </tr>
    );
  }
  ...
  var body = (
    <div style={{ fontFamily: "'DM Sans', sans-serif", display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px" }}>
          <div style={{ position: "relative" }}>
            <button onClick={function() { setDdOpen(function(o) { return !o; }); }}
              ...
              <span style={{ fontSize: 15, fontWeight: 600, color: "#111" }}>{secLabel}</span>
                ...
                {SECCIONES.map(function(s) {
                  return (
                    <div key={s.id} onClick={function() { goTo(s.id); }}
                      style={{ padding: "9px 16px", fontSize: 14, color: seccion === s.id ? colors.accion : "#374151", fontWeight: seccion === s.id ? 600 : 400, cursor: "pointer", background: seccion === s.id ? colors.accionLight : "transparent" }}>
                      {s.label}
                    </div>
                  );
                })}
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 21, cursor: "pointer", color: "#9ca3af", lineHeight: 1 }}>×</button>
        </div>
      </div>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "0 20px 80px" }}>
        ...
        <div ref={function(el) { refs.current["resumen"] = el; }} style={{ paddingTop: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
            <div><Lbl>Estado</Lbl><div style={{ fontSize: 13, color: estadoColor[d.estado] || "#374151" }}>{d.estado}</div></div>
            <div><Lbl>Gestor</Lbl><div style={{ fontSize: 13, color: colors.accion, fontWeight: 500 }}>{d.gestor} {d.gestorRating}★</div></div>
            <div><Lbl>Estrategia</Lbl><div style={{ fontSize: 13, color: "#111" }}>{d.estrategia}</div></div>
            <div><Lbl>Tipo</Lbl><div style={{ fontSize: 13, color: "#111" }}>{d.tipo}</div></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
            <div><Lbl>Meses</Lbl><div style={{ fontSize: 13, color: "#111" }}>{d.meses}</div></div>
            <div><Lbl>Provincia</Lbl><div style={{ fontSize: 13, color: "#111" }}>{d.provincia}</div></div>
            <div><Lbl>Inicio</Lbl><div style={{ fontSize: 13, color: "#111" }}>{d.fechaInicio}</div></div>
          </div>
          <Divider />
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 4 }}>
            <thead>
              <tr>
                <th style={{ textAlign: "left",  color: "#9ca3af", fontWeight: 400, paddingBottom: 5, fontSize: 12 }}></th>
                {["Aport %", "K EUR", "Rep %", "K EUR", "ROI abs", "anual"].map(function(h) {
                  return <th key={h} style={{ textAlign: "right", color: "#9ca3af", fontWeight: 400, paddingBottom: 5, fontSize: 12 }}>{h}</th>;
                })}
              </tr>
            </thead>
            <tbody>
              ...
              <tr style={{ borderTop: "1px solid #f3f4f6" }}>
                <td style={{ padding: "5px 0", fontWeight: 500, color: "#374151", fontSize: 13 }}>Total</td>
                <td></td>
                <td style={{ textAlign: "right", color: "#6b7280", fontSize: 13 }}>{tApK}</td>
                <td></td>
                <td style={{ textAlign: "right", color: "#6b7280", fontSize: 13 }}>{tRepK}</td>
                <td style={{ textAlign: "right", color: "#111", fontSize: 13 }}>{roiAbs(tRepK, tApK)}</td>
                <td style={{ textAlign: "right", color: "#111", fontSize: 13 }}>{roiAn(roiAbs(tRepK, tApK), d.meses)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div ref={function(el) { refs.current["equipo"] = el; }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
             ...
             .map(function(x) {
               return <div key={x[0]}><Lbl>{x[0]}</Lbl><div style={{ fontSize: 14, color: "#374151" }}>{x[1]}</div></div>;
             })}
          </div>
        </div>
        ...
        <div ref={function(el) { refs.current["gestion"] = el; }}>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Estado actual</div>
            <div style={{ marginBottom: 8 }}><Lbl>Valoración en venta</Lbl><div style={{ fontSize: 16, fontWeight: 600, color: "#111" }}>{d.estadoActual.valoracion}</div></div>
            ...
            <div style={{ width: "100%", height: 80, background: "#e5e7eb", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#9ca3af", marginBottom: 8 }}>Mapa</div>
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Problema y solución</div>
            <div style={{ marginBottom: 8 }}><Lbl>Problema</Lbl><Txt text={d.problema} /></div>
            <div><Lbl>Solución</Lbl><Txt text={d.solucion} /></div>
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Fases de la operación</div>
              ...
              {d.fases.map(function(f, i) {
                return (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr", gap: 8, padding: "8px 10px", background: "#f9fafb", borderRadius: 8 }}>
                    <div><Lbl>Fase</Lbl><div style={{ fontSize: 13, fontWeight: 600, color: "#111" }}>{f.fase}</div></div>
                    <div><Lbl>Acciones</Lbl><div style={{ fontSize: 13, color: "#374151", lineHeight: 1.5 }}>{f.acciones}</div></div>
                    <div><Lbl>Duración</Lbl><div style={{ fontSize: 13, color: "#6b7280" }}>{f.duracion}</div></div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Situación actual y próximos pasos</div>
          ...
          {d.estrategiaVenta ? (
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Estrategia de venta</div>
        ...
        <div ref={function(el) { refs.current["analisis"] = el; }}>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Por qué es rentable</div>
            <Txt text={d.porQueRentable} />
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Desglose de la inversión</div>
                  ...
                  {tabla.map(function(row) {
                    return (
                      <div key={row[0]} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #f9fafb" }}>
                        <span style={{ fontSize: 13, color: "#374151" }}>{row[0]}</span>
                        <span style={{ fontSize: 13, color: "#111", fontWeight: ti === 2 ? 600 : 400 }}>{row[1]}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Destino del activo y por qué</div>
            <Txt text={d.destino} />
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Escenarios</div>
          ...
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Fiscalidad de la compraventa</div>
            <Txt text={d.fiscalidadCompra} />
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Prevención de blanqueo de capitales</div>
            <span style={{ display: "inline-block", background: d.pbc === "Necesaria" ? "#fef3c7" : "#f0fdf4", borderRadius: 20, padding: "3px 12px", fontSize: 13, fontWeight: 600, color: d.pbc === "Necesaria" ? "#92400e" : "#166534" }}>{d.pbc}</span>
          </div>
        </div>
        ...
        <div ref={function(el) { refs.current["precio"] = el; }}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 10 }}>Paso 0 · Ficha del inmueble</div>
            ...
            {pv.paso0 && pv.paso0.cargas && pv.paso0.cargas.filter(function(c) { return !!c.acreedor; }).length > 0 && (
              <div style={{ marginTop: 8 }}>
                ...
                {pv.paso0.cargas.filter(function(c) { return !!c.acreedor; }).map(function(c, i) {
                  return <div key={i} style={{ fontSize: 13, color: "#374151" }}>{c.acreedor}: {c.importe}</div>;
                })}
              </div>
            )}
          </div>
          {pv.paso1 && pv.paso1.filter(function(x) { return !!x.agencia; }).length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 10 }}>Paso 1 · Comparables vendidos</div>
              {pv.paso1.filter(function(x) { return !!x.agencia; }).map(function(item, i) {
                return (
                  <div key={i} style={{ padding: "10px 12px", background: "#f9fafb", borderRadius: 8, marginBottom: 6 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: "#111" }}>{item.agencia}</span>
                      <span style={{ fontSize: 14, fontWeight: 700, color: colors.accion }}>{item.precioVenta ? item.precioVenta.toLocaleString() + " €" : ""}</span>
                    </div>
                    <div style={{ fontSize: 13, color: "#6b7280" }}>{item.direccion} · {item.contacto} · {item.telefono}</div>
                  </div>
                );
              })}
            </div>
          ...
          {pv.paso2 && pv.paso2.filter(function(x) { return !!x.titulo; }).length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 10 }}>Paso 2 · Comparables en venta</div>
              {pv.paso2.filter(function(x) { return !!x.titulo; }).map(function(item, i) {
                return (
                  <div key={i} style={{ padding: "10px 12px", background: "#f9fafb", borderRadius: 8, marginBottom: 6 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: "#111" }}>{item.titulo}</span>
                      <span style={{ fontSize: 14, fontWeight: 700, color: colors.accion }}>{item.precio ? item.precio.toLocaleString() + " €" : ""}</span>
                    </div>
                    ...
                    {item.comentarios && <div style={{ fontSize: 13, color: "#9ca3af", marginTop: 3 }}>{item.comentarios}</div>}
                  </div>
                );
              })}
            </div>
          ...
          {pv.paso3 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Paso 3 · Conclusiones</div>
              <div style={{ padding: "12px 14px", background: "#f9fafb", borderRadius: 8, fontSize: 14, color: "#374151", lineHeight: 1.6 }}>
          ...
          {pv.paso4 && (
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 10 }}>Paso 4 · Seguimiento del anuncio</div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, whiteSpace: "nowrap" }}>
                    ...
                    <tbody>
                      ...
                      <tr style={{ borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td></td>
                        {pv.paso4.dias.map(function(dia, i) {
                          return <td key={i} style={{ padding: "4px 8px", textAlign: "right", color: "#9ca3af", fontSize: 11 }}>{dia.fecha}</td>;
                        })}
                      </tr>
                    </tbody>
                  </table>
              ...
              {pv.paso4.comentarios && (
                <div style={{ padding: "10px 12px", background: "#fef3c7", borderRadius: 8, fontSize: 13, color: "#92400e" }}>{pv.paso4.comentarios}</div>
              )}
            </div>
          )}
        </div>
            ...
            {d.documentos.filter(function(doc) { return !!doc.nombre; }).map(function(doc, i) {
              return (
                <button key={i}
                  ...
                  <span style={{ fontSize: 13, color: "#9ca3af", fontWeight: 500 }}>PDF</span>
                  <span style={{ fontSize: 14, color: "#374151", fontWeight: 500 }}>{doc.nombre}</span>
                </button>
              );
            })}
          </div>
        ...
        <div ref={function(el) { refs.current["observaciones"] = el; }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <button onClick={function() { setSubvista("observaciones"); }} style={{ background: "none", border: "none", cursor: "pointer", color: colors.accion, fontSize: 13, fontWeight: 600, fontFamily: "inherit", padding: 0 }}>
          ...
          {d.observaciones.slice(0, 2).map(function(o, i) {
            return (
              <div key={i} style={{ padding: "10px 0", borderBottom: "1px solid #f3f4f6" }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: o.esGestor ? colors.accion : "#111", marginBottom: 3 }}>{o.autor}</div>
                <div style={{ fontSize: 14, color: "#374151" }}>{o.texto}</div>
              </div>
            );
          })}
          <button onClick={function() { setSubvista("observaciones"); }} style={{ marginTop: 10, background: "none", border: "none", cursor: "pointer", color: colors.accion, fontSize: 14, fontWeight: 600, fontFamily: "inherit", padding: 0 }}>
            Ver todas ({d.observaciones.length}) →
          </button>
        </div>
        ...
        <div ref={function(el) { refs.current["resenas"] = el; }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <button onClick={function() { setSubvista("resenas"); }} style={{ background: "none", border: "none", cursor: "pointer", color: colors.accion, fontSize: 13, fontWeight: 600, fontFamily: "inherit", padding: 0 }}>
              Ver todas ({d.resenas.length}) →
            </button>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <Stars n={d.gestorRating} />
            <span style={{ fontSize: 14, fontWeight: 700, color: "#111" }}>{d.gestorRating}</span>
            <span style={{ fontSize: 13, color: "#9ca3af" }}>({d.gestorReviews} valoraciones)</span>
          </div>
          ...
          <button onClick={function() { setSubvista("resenas"); }} style={{ marginTop: 10, background: "none", border: "none", cursor: "pointer", color: colors.accion, fontSize: 14, fontWeight: 600, fontFamily: "inherit", padding: 0 }}>
            Ver todas ({d.resenas.length}) →
          </button>
        </div>
      ...
      <div style={{ padding: "12px 20px", borderTop: "1px solid #f3f4f6", display: "flex", gap: 10, flexShrink: 0, background: "#fff" }}>
        <button style={{ flex: 1, padding: "10px", border: "1.5px solid #e5e7eb", borderRadius: 8, background: "#fff", fontSize: 14, fontFamily: "inherit", cursor: "pointer", color: "#374151", fontWeight: 500 }}>Ver presentación</button>
        <button style={{ flex: 1, padding: "10px", border: "none", borderRadius: 8, background: colors.accion, fontSize: 14, fontFamily: "inherit", cursor: "pointer", color: "#fff", fontWeight: 600 }}>Solicitar invertir</button>
      </div>
